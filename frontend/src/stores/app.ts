...
export const useAppStore = defineStore('app', () => {
  ...
  async function loadProjectData(projectId = currentProjectId.value) {
    if (!projectId) return
    const [memberResult, wbsResult, riskResult, qualityResult, mappingResult, linkResult, taskResult, dailyResult, draftResult, fillResult, attachmentResult, logResult, settingsResult] = await Promise.all([
