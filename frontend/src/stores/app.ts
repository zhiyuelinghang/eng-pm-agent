...
type ApiTask = { id: number | string; project_id: number; title: string; task_type: Task['type']; risk_level: Task['riskLevel']; assignee_user_id?: number; confirmer_user_id?: number; due_at?: string; wbs_item_id?: number; risk_source_id?: number; trigger_reason?: string; required_materials: string[]; workflow_steps?: Task['workflowSteps']; status: string; created_at: string; updated_at?: string; closed_at?: string }
...
export type AttachmentRecord = { id: string; projectId: string; fileName: string; category: string; version: number; fileSize: number; contentType: string; createdAt: string; folderId?: string; snippet?: string; contentPreview?: string }
...
type ApiAttachment = { id: number; project_id: number; file_name: string; category: string; version: number; file_size: number; content_type?: string; created_at: string; folder_id?: number | null; snippet?: string; content_preview?: string }
...
export const useAppStore = defineStore('app', () => {
  ...
  function mapAttachment(row: ApiAttachment): AttachmentRecord { return { id: id(row.id), projectId: id(row.project_id), fileName: row.file_name, category: row.category, version: row.version, fileSize: row.file_size, contentType: row.content_type || '', createdAt: row.created_at, folderId: id(row.folder_id) || undefined, snippet: row.snippet, contentPreview: row.content_preview || '' } }
  ...
  async function updateTaskStep(taskId: string, stepIndex: number, taskStatus: 'pending' | 'processing' | 'completed' | 'blocked', note?: string, attachments?: string[]) { await api.post(`/tasks/${taskId}/steps/${stepIndex}`, { status: taskStatus, note, attachments: attachments || [] }); await loadProjectData() }
  ...
  async function downloadAttachment(attachmentId: string, fileName: string) {
    const response = await api.get<Blob>(`/attachments/${attachmentId}/download`, { responseType: 'blob' })
  ...
  async function fetchAttachmentBlobUrl(attachmentId: string): Promise<string | null> {
    try {
      const response = await api.get<Blob>(`/attachments/${attachmentId}/download`, { responseType: 'blob' })
