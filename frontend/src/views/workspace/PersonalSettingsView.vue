  ...
  <main class="personal-settings-page">
    <section class="personal-settings-shell">
      <div class="personal-settings-layout">
        ...
        <section ref="settingsMainRef" class="settings-main">
          ...
          <section v-else-if="activeSection === 'security'" class="settings-section">
            <header class="section-head">
              <div><span>密码管理</span><h2>修改登录密码</h2><p>修改成功后，当前会话继续有效；下次登录请使用新密码。</p></div>
...
const settingSections = computed(() => [
  { key: 'account' as const, label: '基础信息', description: '姓名、岗位与联系方式', icon: UserCircle },
  { key: 'security' as const, label: '密码管理', description: '修改登录密码与安全提示', icon: ShieldLock },
  ...connectors.map(item => ({
    key: item.key,
    label: item.label,
    description: item.configured ? '已配置' : '尚未配置',
    icon: item.icon,
  })),
])
...
async function loadConnectorSettings() {
  ...
  try {
    const response = await api.get<ApiEnvelope<ApiConnectorConfig[]>>('/me/connectors')
    for (const value of response.data.data) {
      const connector = connectors.find(item => item.key === value.connector_type)
      if (!connector) continue
      connector.account = value.account_identifier || ''
    ...
    message.error(error.response?.data?.detail || '个人连接配置加载失败。')
  } finally {
...
async function saveConnector() {
  ...
  try {
    ...
    message.error(error.response?.data?.detail || '个人连接配置保存失败。')
  } finally {
...
.personal-settings-shell { display: grid; width: 100%; height: 100%; min-height: 0; grid-template-rows: minmax(0, 1fr); overflow: hidden; border: 1px solid #dce6e3; border-radius: 12px; background: #fff; box-shadow: 0 16px 36px rgba(24, 58, 52, .07); }
.section-head > div > span { color: #0f766e; font-size: 12px; font-weight: 800; letter-spacing: .04em; }.section-head p { margin: 0; color: #667c77; font-size: 13px; line-height: 1.55; }
...
.settings-main { min-width: 0; min-height: 0; overflow-x: hidden; overflow-y: auto; padding: 24px 28px 32px; scrollbar-gutter: stable; }.settings-section { display: grid; gap: 22px; }.section-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 20px; padding-bottom: 18px; border-bottom: 1px solid #e2e9e7; }.section-head h2 { margin: 4px 0 5px; color: #173235; font-size: 20px; letter-spacing: -.015em; }.section-head em { display: inline-flex; align-items: center; gap: 5px; flex: 0 0 auto; margin-top: 6px; border-radius: 5px; padding: 5px 8px; color: #53736d; background: #edf4f2; font-size: 12px; font-style: normal; }.section-head em.security-badge,.section-head em.configured { color: #0f766e; }
.settings-form { display: grid; gap: 16px; }.account-form { grid-template-columns: repeat(2, minmax(0, 1fr)); }.settings-form label { display: grid; gap: 7px; color: #49645f; font-size: 13px; font-weight: 700; }.settings-form input,.connector-form select { box-sizing: border-box; width: 100%; min-width: 0; min-height: 42px; border: 1px solid #cad9d5; border-radius: 7px; padding: 9px 11px; color: #193b37; background: #fff; font: inherit; font-size: 13px; outline: 0; transition: border-color .18s ease, box-shadow .18s ease; }.settings-form input:focus,.connector-form select:focus { border-color: #4e9187; box-shadow: 0 0 0 3px rgba(15,118,110,.1); }.settings-form input:disabled { color: #788984; background: #f1f4f3; cursor: not-allowed; }
