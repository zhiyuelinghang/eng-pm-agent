"""把已安装的任务引擎 MCP 分配给唯一启用的 Dobby 全局总控。"""
...
async def assign() -> None:
    ...
    if not any(
        ...
        raise RuntimeError("任务引擎 MCP 尚未安装，不能分配")
    ...
    async with AsyncSQLAlchemyStorage(
        ...
        if "task-engine" in allowed:
            print(f"{target.data.name} 已分配任务引擎 MCP，无需重复修改")
            return
        allowed.append("task-engine")
        ...
        if (
            verified is None
            or "task-engine" not in verified.data.mcp_config.allowed_mcp_ids
        ):
            raise RuntimeError("任务引擎 MCP 分配结果未能持久化")
    print(f"已给 {target.data.name} 分配任务引擎 MCP")
