...
if exist "%ROOT%一键停止全部服务.bat" (
    ...
    if errorlevel 1 (
        echo [失败] 旧服务未能安全停止；为保护其他程序，本次启动已取消。
        goto FAILED
    )
)
...
start "Dobby 工程管理平台" /D "%ROOT%" cmd.exe /c ""%ROOT%start-frontend.bat" --no-browser --no-pause"
...
"%POWERSHELL_EXE%" -NoProfile -ExecutionPolicy Bypass -File "%PROCESS_CONTROL%" -Action RegisterPorts -ProjectRoot "%PROJECT_ROOT%" -Ports "%PLATFORM_WEB_PORT%,%PLATFORM_API_PORT%,%DOBBY_REALTIME_PORT%,%AGENTSCOPE_PORT%,%AGENTSCOPE_WEBUI_PORT%,23000" -WaitSeconds 10 -Quiet
if errorlevel 1 (
...
echo [群聊实时服务] ws://127.0.0.1:%DOBBY_REALTIME_PORT%/connection/websocket
echo.
