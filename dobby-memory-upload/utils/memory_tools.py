...
async def _execute_search_memory(
    ...
    try:
        ...
        def _sync_search():
            ...
            m = get_mem0()
            return m.search(query, filters={"user_id": uid, "agent_id": aid}, top_k=limit, threshold=0.3)
...
async def _execute_search_experiences(
    ...
    try:
        from .lifecycle import _ensure_embeddings, _coarse_filter
        from .config import (
            EXPERIENCE_COARSE_FILTER_THRESHOLD,
            EXPERIENCE_MERGE_CONFIRM_THRESHOLD,
        )
        import psycopg

        db_url = _cfg.LANGGRAPH_CHECKPOINT_DB
        conn = psycopg.Connection.connect(db_url, autocommit=True, prepare_threshold=0)
        ...
        with conn:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    f"SELECT id, body_md, task_type, version, score "
                    f"FROM experience_registry {where} "
        ...
        # Coarse filter via HNSW if embedding available
        model = _ensure_embeddings()
        if model and rows:
            query_vec = model.encode([query], show_progress_bar=False)[0]
            bodies = [r[1] or "" for r in rows]
            filtered = _coarse_filter(
                query_vec, bodies, EXPERIENCE_COARSE_FILTER_THRESHOLD, rows
            )
            rows = filtered or rows

        lines = []
        for r in rows[:5]:
            eid, body, ttype, ver, score = r
            body_preview = (body or "")[:300]
            lines.append(
                f"[经验 #{eid}] 类型: {ttype or '通用'} v{ver} "
                f"得分: {score:.2f}\n{body_preview}"
            )
        return "\n\n".join(lines)
    except Exception as e:
...
async def execute_tool(
    ...
    tool_map = {
        ...
        "search_knowledge_base": lambda: _execute_search_knowledge_base(
            query=arguments.get("query", ""),
            kb_names=kb_names or [arguments.get("kb_name")] if arguments.get("kb_name") else None,
        ),
