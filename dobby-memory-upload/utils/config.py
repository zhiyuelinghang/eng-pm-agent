...
# ---- Embedding ----
EMBEDDING_PROVIDER = _env("EMBEDDING_PROVIDER", "local")  # local | dashscope
EMBEDDING_MODEL = _env("EMBEDDING_MODEL", "BAAI/bge-large-zh-v1.5")
EMBEDDING_DIMS = 1024  # bge-large-zh-v1.5 = 1024维
...
MEM0_INFER_ENABLED = _env("MEM0_INFER_ENABLED", "false").lower() == "true"
...
LANGGRAPH_CHECKPOINT_DB = _env(
    "LANGGRAPH_CHECKPOINT_DB",
    "postgresql://dobby:dobby@localhost:5432/dobby_demo",
)
COMPRESSION_TRIGGER_TOKENS = 160_000  # 80% of 200K context
...
LLM_FLASH_MODEL = _env("LLM_FLASH_MODEL", "deepseek-v4-flash")
# 强力模型 — 用于用户直接看到的回答和最终合成
LLM_PRO_MODEL = _env("LLM_PRO_MODEL", "deepseek-v4-pro")
...
MAX_TOKEN_BUDGET = 200_000                 # 总窗口上限
...
def validate() -> list[str]:
    ...
    elif EMBEDDING_PROVIDER == "dashscope" and not DASHSCOPE_API_KEY:
        issues.append(
            "DashScope embedding requires DASHSCOPE_API_KEY."
        )
...
Configuration:
  ...
  Database:      {DATABASE_URL[:DATABASE_URL.find('@')]}
  ...
  RRF:           KB_w={FUSION_KB_WEIGHT}, LTM_w={FUSION_MEM0_WEIGHT}, k={RRF_K}
	  PostgresSaver: {LANGGRAPH_CHECKPOINT_DB[:LANGGRAPH_CHECKPOINT_DB.find('@')]} (compress@{COMPRESSION_TRIGGER_TOKENS}tk)
