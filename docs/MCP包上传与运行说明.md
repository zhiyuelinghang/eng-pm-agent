...
- `platform_capabilities`：可选的平台托管能力声明。普通 MCP 保持空数组；受信业务包
  ...
  标准数据库交互接口；`task-engine` 另有专用数据库能力，见下文。项目初始化智能体
  直接使用平台数据库交互，跨分区业务核验由平台调用专用核验 MCP；
...
普通 MCP 不获得数据库连接。声明 `dobby_database_interactions` 的受信业务包通过
平台接口读取或修改已分配范围内的记录；每次写入重新检查表白名单，并由平台补充项目、
会话、用户、执行智能体和审计信息。该接口不提供数据库账号或原始 SQL 执行能力。

`task-engine` 是专用例外：只有该包可以申请 `dobby_task_engine_store`，平台向其进程
注入 `TASK_ENGINE_DATABASE_URL`、`TASK_ENGINE_SCHEMA` 和时区，与平台后端共用
PostgreSQL 任务引擎 schema。该进程直接连接数据库，不具有上述数据库交互接口的逐表
白名单隔离，必须按受信本地程序管理；连接信息不进入浏览器或模型上下文。具体约束见
[清单校验](../AgentScope/agentscope/app/mcp_registry/_models.py)、
[环境注入](../AgentScope/agentscope/app/mcp_registry/_manager.py) 和
[任务引擎接入指南](../mcp-packages/task-engine/Dobby接入指南.md)。

## 4. 上传、升级与分配

1. 在“平台主智能体”或“业务智能体”页面选择目标智能体，切换到 `MCP` 页签；
...
初始化文档解析按页读取，不以文档数量或 MinerU 结果 ZIP 内的文件数量截断资料，
但会校验解压后总大小。原生图片输入另受数量、大小与模型能力限制，详见
[附件解析工具](../mcp-packages/attachment-parser/README.md)。这些限制与 MCP 包的
上传、解压大小限制分别生效。
