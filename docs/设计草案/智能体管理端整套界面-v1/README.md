...
日期：2026-09-08。基于已选择的第 1 版「职责导航工作台」，本套保留 14 张原始设计图。状态：配置与调试改版已在本地真实管理端实施；按用户最终要求，第 14 张新增运行记录功能取消，图纸仅留档。

[打开图集](./index.html) · [功能覆盖与源码核对](../智能体管理端入口与职责调整讨论稿.md#7-本轮确认的弹窗与功能覆盖)

**用户复核后的最新调整**：主智能体直接配置自身，移除“职责绑定”；MCP 等能力改成真实页面内的上方 Tab 和下方列表，查看详情才弹窗。详情加宽、目录名称允许换行；初始化规则版本在“初始化核验”中维护。原始图纸继续留档，相关弹窗形态以[最新修订](../智能体管理端入口与职责调整讨论稿.md#9-用户复核后的修订直接配置页面-tab单层详情)为准。
...
| 14 | [运行记录](./14-运行记录.png) | 已取消实施，仅保留原始设计 |
...
独立菜单、独立页面，复用主智能体的列表与详情布局，支持搜索和新增业务工具。当前通过实际职责绑定区分两个列表，保留未发布和内部协同对象。
...
技能分配、创建、编辑、内容预览、历史版本、下载和删除均保留。落地复用既有资源详情与编辑弹窗，关闭资源弹窗后返回能力配置，保留分配草稿。
...
用户最终要求本次关注配置能力与调试，本图所示新增运行记录页和详情标签取消实施。既有平台交互审计保留在基础配置区域；调试仍保留会话历史、思考、工作过程与结果。
...
8. 实际编辑器为四个分组，专用模型参数、上下文和 ReAct 折叠；能力配置复用现有资源目录及其详情弹窗，没有重写为图中的左右分栏资源浏览器。模型字段与复选框沿用现有表单组件。
9. 调试使用真实会话接口与已有消息组件，不提供图中示意的项目上下文选择器或新增运行详情页。项目资料权限仍由实际业务入口的用户和项目上下文控制。

## 实施与验收

- 基线提交 `c726928a` 已推送到指定仓库 `main`，随后开始本地改版。
- 首页为 `/platform-agents`，第二菜单为 `/business-tools`；旧 `/platform-settings` 跳转首页，已有具体聊天链接继续有效。
- 本地真实页面已验证新增、编辑、草稿保护、MCP 分配保存与回显、调试消息及历史、会话重命名、删除临时工具。验收临时工具已删除；原有职责和权限未重配。
- 前端构建、4 项列表与职责状态测试、61 项接入与权限相关测试通过。全量测试受现有数据分析依赖缓存缺失阻塞，结构检查存在两处本轮未修改文件的行数超限。
- 完整截图对照、限制及验证记录见 [design-qa.md](../../../design-qa.md)。
