...
def should_trigger_historian(
    ...
    *,
    ...
    threshold = _cfg.HISTORIAN_TRIGGER_TOKENS
    if max_token_budget and trigger_ratio:
        threshold = max(1, int(max_token_budget * trigger_ratio))
    return uncovered_tokens >= threshold
...
async def produce_compartment(
    ...
    if _call_model_fn is None:
        ...
        async def _historian_model(msgs):
            return await _call_model(msgs, intent="historian")

        _call_model_fn = _historian_model
...
async def historian_cycle(
    ...
    if not should_trigger_historian(
        messages,
        compartments,
        max_token_budget=state.get("max_token_budget"),
        trigger_ratio=state.get("historian_trigger_ratio"),
    ):
        return None
