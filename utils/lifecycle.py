...
async def _ensure_embeddings(extracts: list[dict]) -> None:
    ...
    vectors = None
    try:
        if not _cfg.EMBED_SERVER_ENABLED:
            raise RuntimeError("embed server is disabled")
        import httpx
        ...
        # Reuse Mem0's process-wide embedder instead of loading a second
        # SentenceTransformer instance inside the AgentScope service.
        try:
            from .langgraph_utils import get_mem0

            embedder = get_mem0().embedding_model
            if hasattr(embedder, "embed_batch"):
                raw_vectors = embedder.embed_batch(texts, "add")
            else:
                raw_vectors = [embedder.embed(text, "add") for text in texts]
