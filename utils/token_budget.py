...
class TokenBudget:
    ...
    def __init__(
        ...
        *,
        ...
        self._layer_limits = dict(layer_limits or {})
    ...
    def _max_tokens(self, name: str, cfg: dict[str, Any]) -> int | None:
        if name in self._layer_limits:
            return self._layer_limits[name]
        max_tokens_fn = cfg.get("max_tokens")
        return max_tokens_fn() if max_tokens_fn is not None else None
    ...
    def allocate(self, layers: dict[str, Any]) -> BudgetAllocation:
        ...
        for name, content in layers.items():
            ...
            max_tokens = self._max_tokens(name, cfg)
