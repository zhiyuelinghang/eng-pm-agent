...
async def _get_graphiti(project_id: str):
    ...
    Uses the selected platform memory model for LLM work and local
    BGE-large-zh-v1.5 for embeddings. Without an explicit selection, the
    legacy DeepSeek service configuration remains the fallback.
    """
    if not _cfg.GRAPHITI_ENABLED:
        return None
    try:
        ...
        llm_config = LLMConfig(
            ...
            max_tokens=int(runtime.get("max_tokens", 2000)),
        )
        ...
        elif runtime.get("provider") == "gemini":
        ...
        class MemoryModelReranker(CrossEncoderClient):
            """Score passages with the active memory model without a hidden LLM."""
            ...
            async def rank(
                ...
                async def score(passage: str) -> float:
                    response = await _call_model(
                        [
                            _make_system(
                                "你负责判断候选记忆与查询的相关程度，只返回0到1之间的小数。",
                            ),
                            _make_user(f"查询：{query}\n候选内容：{passage}"),
                        ],
                        intent="extract",
                    )
                    text = _msg_content(response).strip()
                    match = re.search(
                        r"(?<![\d.])(?:0(?:\.\d+)?|1(?:\.0+)?)(?![\d.])",
                        text,
                    )
                ...
                scores = await asyncio.gather(*(score(item) for item in passages))
                return sorted(
        ...
        embedder_config = OpenAIEmbedderConfig(
            api_key=_cfg.EMBEDDING_API_KEY or "not-needed",
            base_url=_cfg.EMBED_SERVER_URL,
        ...
        return Graphiti(
            ...
            embedder=embedder,
            cross_encoder=MemoryModelReranker(),
        )
    except Exception:
        return None
