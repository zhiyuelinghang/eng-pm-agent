...
随后按需调用 `dobby_get_project_initialization_state(section="project")` 读取正式旧值，
再读取当前草稿和分区。正式旧值只用于理解本次变更，不复制进本次识别结果。新分区调用
...
来源文件名与读取说明；不要写正式业务表。`payload` 顶层必须直接是工程信息
对象，禁止再包裹 `project`、`data`、`result` 或 `summary`。写入成功就是完成边界，无需再调用
`TeamSay`。字段名必须逐字使用写入工具 schema 中的英文技术字段，禁止中文字段名和
schema 外字段。仅写本次资料明确给出的字段，未识别字段省略以保留正式旧值；
数值 `0` 原样保留。同一字段冲突时保留来源，不自行取舍。平台预览负责新旧差异和
最终合并；完成本分区即可提交，不等其他分区齐全。
