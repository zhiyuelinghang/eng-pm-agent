...
def _sanitize_schema_for_gemini(schema: Any) -> Any:
    ...
    - ``type`` arrays containing ``"null"``: simplified to one non-null
      type or converted to ``anyOf`` for multiple non-null types.
    ...
    if _is_null_schema(schema):
        ...
        non_null_types = [value for value in schema["type"] if value != "null"]
        if len(non_null_types) == 1:
            ...
            schema.pop("type")
            schema["anyOf"] = [
                {"type": type_} for type_ in non_null_types
            ]
        else:
            schema["type"] = "object"
