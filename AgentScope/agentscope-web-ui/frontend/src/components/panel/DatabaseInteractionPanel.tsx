...
import type { AgentView, DatabaseInteraction, DatabaseTableInteractionRequest } from '@/api';
...
export function DatabaseInteractionPanel({ agent, onDirtyChange, onBusyChange }: Props) {
	...
	useEffect(() => {
		onBusyChange?.(savingAssignments);
	}, [savingAssignments, onBusyChange]);
	...
	if (!agent) {
		return (
			<PanelEmpty
				icon={Database}
				title={t('panel.database.noAgentTitle')}
				description={t('panel.database.noAgentDescription')}
			/>
		);
	}
	...
	return (
		<div className="flex min-h-0 flex-1 flex-col">
			<div className="flex-none space-y-3 pb-3">
					...
					<div className="min-w-0">
						<div className="flex items-baseline gap-1.5">
							<span className="text-sm font-medium">
								{t('panel.database.catalogTitle')}
							</span>
							<span className="text-xs tabular-nums text-muted-foreground">
								{t('panel.database.selectedSummary', {
				...
				<InputGroup>
					<InputGroupInput
						placeholder={t('panel.database.searchPlaceholder')}
						value={search}
			...
			<div className="min-h-0 flex-1 overflow-y-auto rounded-lg border">
				{loading ? (
					...
					<PanelEmpty
						...
						title={
							search ? t('panel.search.emptyTitle') : t('panel.database.emptyTitle')
						}
						description={
							search
								? t('panel.search.emptyDescription', { query: search })
								: t('panel.database.emptyDescription')
						}
					/>
				) : (
						...
						{filtered.map((item) => {
							...
							return (
								<PanelCatalogRow
									...
									metadata={
										<>
											{item.policy?.display_name} ·{' '}
											{item.read_only
												? t('panel.database.readOnly')
												: t('panel.database.write')}
										</>
									}
									badge={
										!item.enabled ? (
											<Badge variant="outline">
												{t('panel.database.disabled')}
											</Badge>
										) : null
									}
									selected={checked}
									...
									checkbox={{
										checked,
										disabled:
											!agent.editable || savingAssignments || !item.enabled,
										ariaLabel: t('panel.database.toggleAssignment', {
											name: item.display_name,
										}),
										onChange: (value) => toggleAssignment(item.id, value),
									}}
									onOpen={() => openDetail(item)}
									openLabel={t('panel.database.viewDetails', {
										name: item.display_name,
									})}
								/>
							);
			...
			<div className="flex-none space-y-2 pt-3">
				{!agent.editable ? (
					<Alert>
						<CircleAlert />
						<AlertDescription>{t('panel.database.readOnlyNotice')}</AlertDescription>
					</Alert>
				) : null}
				{errorMsg ? (
				...
				<div className="flex items-center justify-between border-t pt-3">
					<Button
						...
						disabled={!isDirty || savingAssignments}
					>
						<RotateCcw />
						{t('panel.database.discard')}
					</Button>
					<Button
						size="sm"
						onClick={() => void saveAssignments()}
						disabled={!agent.editable || !isDirty || savingAssignments}
					>
						{savingAssignments ? <Loader2 className="animate-spin" /> : <Save />}
						{t('panel.database.saveChanges')}
					</Button>
				</div>
			</div>
			...
			<DatabaseInteractionDialog
				...
				onSaveTable={async (
					id: number | null,
					payload: DatabaseTableInteractionRequest,
				) => {
					if (id === null) await createInteraction(payload);
					else await updateTableInteraction(id, payload);
				}}
			...
			<DeleteDialog
				...
				title={t('panel.database.deleteTitle')}
				description={t('panel.database.deleteDescription')}
				onConfirm={async () => {
					if (!deleteTarget) return;
					await removeInteraction(deleteTarget.id);
