...
def _require_step_owner(task: TaskInstance, step: Step, actor: str, action: str) -> None:
    """只有当前节点负责人可以推进节点；空 actor 视为系统调用。"""
...
def complete_step(
    ...
    *,
    ...
    _require_step_owner(task, step, actor, "完成")
...
def skip_step(
    ...
    *,
    ...
    _require_in_turn(task, step)
    _require_step_owner(task, step, actor, "跳过")
...
def block_step(
    ...
    *,
    ...
    if step.is_settled:
        raise TransitionError(f"节点「{step.name}」已经了结")
    _require_step_owner(task, step, actor, "阻塞")
...
def unblock_step(
    ...
    *,
    ...
    _require_step_owner(task, step, actor, "解除阻塞")
