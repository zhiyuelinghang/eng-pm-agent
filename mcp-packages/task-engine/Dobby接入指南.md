...
> **本仓库落地说明（2026-08-25）**：总体接入边界仍以本指南为准；按项目要求，
> 引擎存储由 SQLite 替换为平台现有 PostgreSQL 的独立 `task_engine` schema。
> 此外仅增加一项宿主扩展：流程节点可标记为 `automated`。人工节点继续完整执行
> “责任人 + 工点 + 确认人”硬约束；自动节点的具体动作配置写在
> `TaskFlow.scope["step_actions"]`，由 Dobby 宿主执行，引擎仍只负责节点顺序、
> 状态、触发计划和审计。当前的“发送群聊消息”就是该扩展的一种节点动作，
> 不是独立任务模式，也没有把群聊业务写进引擎领域模型。
