"""STDIO entrypoint for the platform's fixed attachment parser tool."""
...
from parser_runtime import AttachmentParserError, parse_attachment


SERVER_NAME = "attachment-parser"
...
def _tool_result(arguments: dict[str, Any]) -> dict[str, Any]:
    try:
        payload = parse_attachment(**arguments)
        return {
            "content": [
                {
                    "type": "text",
    ...
    except (AttachmentParserError, TypeError, ValueError) as exc:
        return {
        ...
        return {
            "content": [{"type": "text", "text": f"附件解析异常：{exc}"}],
            "isError": True,
        }
...
def _handle(message: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if method == "tools/call":
        ...
        if tool is None:
            return _error(request_id, -32602, "未知的附件解析工具")
